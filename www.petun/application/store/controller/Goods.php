...
class Goods extends BasicAdmin
{
    ...
    public function edit()
    {
        ...
        try {
            ...
            Db::transaction(function () use ($data, $goods_id, $goods) {
                ...
                // 更新商品详细
                $old = Db::name('StoreGoodsList')->field("goods_stock,goods_sale")->where(['goods_id' => $goods_id])->select();
                //编辑产品时,会生成新的规格信息，故需要把原来的库存同步过来
                foreach ($data['list'] as $k1 => $v1) {
                    $data['list'][$k1]['goods_stock'] = $old[$k1]['goods_stock'];
                    $data['list'][$k1]['goods_sale'] = $old[$k1]['goods_sale'];
                }
    ...
    protected function _form_build_data()
    {
        ...
        $main['illustrate'] = $this->request->post('illustrate', '');
        $main['desc_left'] = $this->request->post('desc_left', '');
        $main['content_img'] = $this->request->post('content_img', '');
        $main['spec'] = $this->request->post('spec', '');
        $main['is_sale'] = $this->request->post('is_sale', '');
        $main['content_detail'] = $this->request->post('content_detail', '');
        $main['is_banner'] = $this->request->post('is_banner', '');
        $main['banner'] = $this->request->post('banner', '');
        $main['cate_top'] = $this->request->post('cate_top', '');
        $main['is_intro'] = $this->request->post('is_intro', '');
        $main['star'] = $this->request->post('star', '');
